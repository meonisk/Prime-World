[English](README_English.md)        [Русский](README.md)        [中文](README.zh.md)        [हिन्दी](README.hi.md)        [Español](README.es.md)        [Français](README.fr.md)        [Deutsch](README.de.md)        [Português](README.pt.md)        [日本語](README.ja.md)        [Bahasa Indonesia](README.id.md)
...
- `engine/`, `client/src/`, `server/battle/src/` — battle part source
- `client/build (client), server/ (servers)` — compiled client of the battle part with cheat codes and a client editor
- `editor/build/PF_Editor.exe` — game data editor

## Preparation
You need to download the data from this repository and merge the `Bin` folder with the main game data. Here's how to do it:

1. Copy the folder `client/build/Bin` to `(legacy path, now split across engine/, client/, server/, editor/, assets/, localization/, profiles/, vendor/, tools/)`, replacing existing files.
2. Run the client with cheat codes `client/build/Bin/PW_Game.exe`.
...
If any errors occur, check the log files in `logs/`.

## Game Data
Data can be edited via the editor and is located in `assets/`
...
2. `Add -> WinFileSystem`.
3. Set the Data folder as the system root: `assets/`.
4. Close the windows.
...
In the repository, you can find the already compiled game client with cheat codes `client/build/Bin/PW_Game.exe`
