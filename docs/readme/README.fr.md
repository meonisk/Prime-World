...
- `client/build (client), server/ (servers)` — client compilé de la partie combat avec codes de triche et éditeur client
- `editor/build/PF_Editor.exe` — éditeur de données du jeu
...
1. Copiez le dossier `client/build/Bin` dans `(legacy path, now split across engine/, client/, server/, editor/, assets/, localization/, profiles/, vendor/, tools/)`, en remplaçant les fichiers existants.
2. Lancez le client avec les codes de triche `client/build/Bin/PW_Game.exe`.
...
En cas d'erreur, consultez les fichiers journaux dans `logs/`.

## Données du jeu
Les données peuvent être éditées avec l'éditeur situé dans `assets/`
...
L'éditeur de données du jeu se trouve dans `editor/build/PF_Editor.exe`

Lors de la première ouverture de l'éditeur, il faut configurer le chemin vers `Data` :
1. `Tools -> File System Configuration`.
2. `Add -> WinFileSystem`.
3. Définissez le dossier Data comme racine du système : `assets/`.
...
Dans le dépôt, vous pouvez trouver le client déjà compilé avec codes de triche `client/build/Bin/PW_Game.exe`
