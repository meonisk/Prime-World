...
## 本仓库内容
- `pw` — 战斗部分的主代码
- `client/build (client), server/ (servers)` — 已编译含作弊码的战斗客户端及客户端编辑器
- `editor/build/PF_Editor.exe` — 游戏数据编辑器

## 准备工作
需要下载本仓库的数据，并将 `Bin` 文件夹与游戏主数据合并。操作步骤如下：

1. 将 `client/build/Bin` 文件夹复制到 `(legacy path, now split across engine/, client/, server/, editor/, assets/, localization/, profiles/, vendor/, tools/)` 并覆盖已有文件。
2. 用作弊码运行客户端 `client/build/Bin/PW_Game.exe`。
...
如遇到错误，请查看 `logs/` 文件夹下的日志文件。

## 游戏数据
游戏数据可通过编辑器编辑，位于 `assets/`
...
## 游戏数据编辑器
游戏数据编辑器位于 `editor/build/PF_Editor.exe`

首次打开编辑器时需配置 `Data` 路径：
1. `Tools -> File System Configuration`
2. `Add -> WinFileSystem`
3. 设置 Data 文件夹为系统根目录：`assets/`
4. 关闭窗口
...
## 含作弊码的游戏客户端
在仓库中可找到已编译带作弊码的游戏客户端 `client/build/Bin/PW_Game.exe`
