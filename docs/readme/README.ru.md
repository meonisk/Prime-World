...
- `client/build (client), server/ (servers)` — собранный клиент боевой части с чит кодами и редактор для клиента
- `editor/build/PF_Editor.exe` - редактор игровых данных

## Подготовка
Необходимо скачать данные из этого репозитория и объединить папку `Bin` с основными данными игры. Вот как это сделать:

1. Скопируйте папку `client/build/Bin` в `(legacy path, now split across engine/, client/, server/, editor/, assets/, localization/, profiles/, vendor/, tools/)` заменяя сушествуюшие файлы.
2. Запустите клиент с чит кодами `client/build/Bin/PW_Game.exe`.
...
В случае возникновения ошибок посмотрите что написано в лог файлах в `logs/`.

## Данные игры
Данные редактируются через редактор и расположены в `assets/`
...
Редактор игровых данных находится в `editor/build/PF_Editor.exe`

При первом открытии редактора нужно настроить путь к `Data`:
1. `Tools -> File System Configuration`.
2. `Add -> WinFileSystem`.
3. В качестве system root установите папку Data: `assets/`.
4. Закройте окна.
...
В репозитории вы можете найти уже собранный клиент игры с чит кодами `client/build/Bin/PW_Game.exe`
